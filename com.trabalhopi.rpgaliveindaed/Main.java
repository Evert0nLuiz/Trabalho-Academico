    ...
    public static void main(String[] args) {
        ...
        if (resposta == 0) {
            ...
            if (pularBatalha == 0) {
                ...
                int batalha = batalhas.batalha(status.getVida(), status.getEspecial(), status.getForca(), statusCoringa, status.getVidaZumbi(), status.getForcaZumbi());
        ...
        if (resposta == 1) {
                ...
                JOptionPane.showMessageDialog(null, "Você anda alguns metros e se depara com um zumbi, ele reage ao seu cheiro de sangue e parte para cima de você", titulo, JOptionPane.PLAIN_MESSAGE);
                pularBatalha = 0;

            if (pularBatalha == 0) {
                JOptionPane.showMessageDialog(null, "O ZUMBI TE ATACA!!", titulo, JOptionPane.PLAIN_MESSAGE);
                // realizar todo o código da batalha dentro desse IF
                status.criaZumbi();
                Batalhas batalhas = new Batalhas();
               int batalha = batalhas.batalha(status.getVida(), status.getEspecial(), status.getForca(), statusCoringa, status.getVidaZumbi(), status.getForcaZumbi());
        ...
        // inicio batalha
        status.criaBoss();
        int vitoriaBatalha = batalhas.batalha(status.getVida(), status.getEspecial(), status.getForca(),0, status.getVidaBoss(), status.getForcaBoss());
